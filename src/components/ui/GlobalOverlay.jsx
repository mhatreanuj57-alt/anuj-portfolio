...
const GlobalOverlay = () => {
    const { overlayContent, closeOverlay, exitRoom } = useScene();
    ...
    // Clear cached content after exit animation completes
    useEffect(() => {
        if (!overlayContent && cachedContent) {
            const timer = setTimeout(() => setCachedContent(null), 800);
            return () => clearTimeout(timer);
        }
    }, [overlayContent, cachedContent]);
    ...
    return <ContentCard content={content} isOpen={animateOpen} onClose={closeOverlay} onExitRoom={exitRoom} isMobile={isMobile} />;
};
...
const ContentCard = ({ content, isOpen, onClose, onExitRoom, isMobile }) => {
    ...
    const maskStyle = (content.layout === 'certificate_grid' || content.layout === 'badge_sphere' || content.layout === 'carousel_editor') ? {
        maskImage: 'none',
    ...
    // Fullscreen layouts bypass the card entirely
    if (content.layout === 'carousel_editor') {
        const handleCarouselClose = () => {
            onClose();
            // Also exit the room so currentRoom is cleared — allows re-entry via map
            ...
            >
                <div
                    style={{
                        ...
                        } : content.layout === 'badge_sphere' ? {
                            width: isMobile ? '95vw' : 'min(80vw, 800px)',
                            height: isMobile ? '60vh' : 'min(75vh, 700px)',
                            maxHeight: '85vh',
                            left: '50%',
                            top: '50%',
                            right: 'auto',
                ...
                >
                    ...
                    {content.layout === 'certificate_grid' ? (
                            ...
                            <div
                                ...
                                {content.items?.map((item, index) => (
                                    <div key={index} style={{
                                        ...
                                        onClick={(e) => { e.stopPropagation(); if(item.url && item.url !== '#' && !item.url.startsWith('javascript')) window.open(item.url, '_blank'); }}
                                    >
                                        <div style={{
                                            position: 'relative',
                                            width: '100%',
                        ...
                        /* === LAYOUT: 3D BADGE SPHERE === */
                        <BadgeSphereCanvas items={content.items || []} />
                    ) : content.layout === 'carousel_editor' ? (
                        /* === LAYOUT: CAROUSEL EDITOR === */
                        <CarouselEditor onClose={onClose} />
                    ) : (
