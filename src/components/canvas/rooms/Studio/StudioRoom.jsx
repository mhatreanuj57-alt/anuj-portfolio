...
const StudioRoom = ({ showRoom, onReady, isExiting, isWarmup }) => {
    ...
    useEffect(() => {
        const handleWheel = (e) => {
            // Ignore wheel events when an overlay is open
...
// ===========================================
const MonitorBlock = memo(({ item, meshRef, isSelected, onMonitorClick, disabled, paintOnBeforeCompile, paintUniforms, deviceColors }) => {
    ...
    // Face color keys in order: [right, left, top, bottom, front, back]
    const FACE_KEYS = ['right', 'left', 'top', 'bottom', 'front', 'back'];
    const deviceColor = deviceColors?.[deviceShape] || null;
    ...
    const paintedMaterials = useMemo(() => {
        ...
        return faceConfig.map((f, i) => {
            const faceColor = deviceColor?.[FACE_KEYS[i]] || '#e0e0e0';
    ...
    }, [faceConfig, paintOnBeforeCompile, deviceColor]);
    ...
    const sketchMaterials = useMemo(() => {
        ...
        return faceConfig.map((f, i) => {
            ...
            const faceColor = deviceColor?.[FACE_KEYS[i]] || '#e0e0e0';
            const mat = new THREE.MeshBasicMaterial({ color: faceColor, map: f.sketch });
            // Apply paint transition shader
            if (paintOnBeforeCompile) {
                mat.onBeforeCompile = paintOnBeforeCompile;
            ...
            <mesh frustumCulled={false}>
                ...
                {faceConfig.map((face, i) => {
                    ...
                    if (face.painted) {
                        ...
                        return (
                            <revealMaterial color={faceColor}
                                key={`s${i}`}
                                ref={matRefs[i]}
                                attach={`material-${i}`}
                                map={face.sketch}
